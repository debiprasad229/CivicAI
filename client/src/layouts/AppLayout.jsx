    ...
    <div className="min-h-screen bg-slate-50 flex flex-col antialiased text-slate-900">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-2xs">
        <div className="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          {/* Right: Role Switcher Demo Badge, Notifications & Profile */}
          <div className="flex items-center gap-3">
            {/* Quick Role Switcher Button for Demo */}
            <button
              onClick={handleToggleRole}
              title="Toggle between Citizen and Municipal Authority views"
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-colors bg-white hover:bg-slate-50 border-slate-200 text-slate-700 shadow-2xs"
            >
              <ArrowLeftRight className="w-3.5 h-3.5 text-blue-600" />
              <span>Switch to {isCitizen ? 'Municipal Admin' : 'Citizen'}</span>
            </button>
            ...
            <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white border-r border-slate-200">
              ...
              <div className="p-4 bg-slate-50 border-b border-slate-200">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-white border border-slate-200 text-xs font-semibold text-slate-700 shadow-xs"
                >
                  <ArrowLeftRight className="w-3.5 h-3.5 text-blue-600" />
                  Switch to {isCitizen ? 'Municipal Admin' : 'Citizen'}
                </button>
              </div>
